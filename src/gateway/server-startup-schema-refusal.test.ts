...
it("reports the bootstrap schema refusal once before exit 78", async () => {
  ...
  try {
    ...
    const runtimeError = vi.spyOn(defaultRuntime, "error").mockImplementation(() => {});
    ...
    expect(output).toContain(
      JSON.stringify(
        doctor.message.replace("Doctor refused to continue", "Gateway refused startup"),
      ).slice(1, -1),
    );
    expect(errorLog).toHaveBeenCalledOnce();
    expect(errorLog.mock.invocationCallOrder[0]).toBeLessThan(exit.mock.invocationCallOrder[0]!);
